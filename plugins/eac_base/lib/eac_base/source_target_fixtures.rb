  ...
  class SourceTargetFixtures
    ...
    def source_target_files
      sources_targets_basenames.each do |test_basename|
        yield(source_file(test_basename), target_file(test_basename))
      end
    end
    ...
    def fixture_file(basename, suffix)
      ...
      raise "\"#{prefix}\" not found (Basename: #{basename}, Suffix: #{suffix})"
    end
    ...
    def sources_targets_basenames
      ...
      raise "\"#{fixtures_directory}\" não possui nenhum arquivo para teste." if basenames.empty?
      basenames
    end
  end
end
